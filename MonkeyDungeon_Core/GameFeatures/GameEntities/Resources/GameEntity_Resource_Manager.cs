...
namespace MonkeyDungeon_Core.GameFeatures.GameEntities.Resources
{
    public class GameEntity_Resource_Manager
    {
        ...
        public T Get_ResourceByType<T>                          (string resourceName = null) where T : GameEntity_Resource { foreach (T resource in RESOURCES.OfType<T>()) if (resource.ATTRIBUTE_NAME == resourceName) return resource; return null; }
        public GameEntity_Resource Get_Resource                 (string resourceName) => Get_ResourceByType<GameEntity_Resource>(resourceName);
        public double Get_Resource_Percentage                   (string resourceName) { GameEntity_Resource r = Get_Resource(resourceName); return r.Value / r.Max_Quantity; }
